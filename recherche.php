...
$allProduit = $con->query('SELECT * FROM products ORDER BY product_id DESC');
if (isset($_GET['search']) and !empty($_GET['search'])) {
    # code...
    $recherche = htmlspecialchars($_GET['search']);
    $allProduit = $con->query('SELECT * FROM products WHERE product_title LIKE "%' . $recherche . '%" ORDER BY id DESC');
}
...
<body>
            ...
            <div class="col-lg-4 col-6 text-left">
                <form method="GET" action="recherche.php?search=<?php $_GET['search'] ?>">
                    <div class="input-group">
                        <input type="text" class="form-control" name="search" placeholder="Rechercher un Produit" value="<?= $_GET['search']; ?>">
    ...
    <div class="row px-xl-5">
        <?php
        $rowCnt = mysqli_num_rows($allProduit);
        if ($rowCnt > 0) {
            # code...
            while ($produit = mysqli_fetch_assoc($allProduit)) {
                # code...
        ?>
